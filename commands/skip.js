...
module.exports = {
    ...
    run: function (message, prefix, args, client) {
        ...
        if (serverQueue.skips >= Math.round(serverQueue.voiceChannel.members.find(x => x.user.bot == false).length / 2)) {
            serverQueue.player.stop();
            message.reply('Geskippt! :thumbsup:')
        } else {
            message.reply(`Abgestimmt! (${serverQueue.skips}/${Math.round(serverQueue.voiceChannel.members.find(x => x.user.bot == false).length / 2)})`)
